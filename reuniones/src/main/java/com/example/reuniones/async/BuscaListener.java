...
public class BuscaListener {
    ...
    public void recibirMensaje(String mensaje) {
        try {
            ...
            Optional<Persona> persona = Optional.ofNullable(personaService.getById(info.getIdAsistente()));
            if (persona.isEmpty()) {
                LOG.warn("Mensaje recibido, pero la persona {} no existe",
                        info.getIdAsistente());
            }
            Optional<Reunion> reunion = Optional.ofNullable(reunionService.getById(info.getIdReunion()));
            if (reunion.isEmpty()) {
